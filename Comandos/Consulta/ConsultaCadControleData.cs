...
{
    public class ConsultaCadControleData
    {
        ...
        public CadControleLista ListaCadControleData(DateTime dateTime)
        {
            ...
            SQL = "SELECT tblControle.ID, tblControle.DataHoraSaida, tblControle.DataHoraChegada, tblControle.Descricao, tblCarro.Marca, tblMotorista.Nome " +
                  ...
                  "WHERE  tblControle.DataHoraSaida LIKE @DataSaida " +
                  "ORDER BY tblControle.DataHoraSaida DESC ";
            try
            {
                conexaoBancoDados.LimparParametros();
                conexaoBancoDados.AdicionarParametros("@DataSaida", "%" + dateTime.Date.ToString("yyyy-MM-dd") + "%");
